...
[data2b] = thingSpeakRead(readChannelID2,'Fields',1:8,...
                          'DateRange',dateRange,'outputformat','timetable');   
...
[data2c] = thingSpeakRead(readChannelID2,'Fields',1:8,...
                          'DateRange',dateRange,'outputformat','timetable');   
...
endDate = 'January 10, 2019 ';
dateRange = [datetime(startDate),datetime(endDate)];
% Read data including the timestamp, and channel information.
[data1d] = thingSpeakRead(readChannelID1,'Fields',1:8,...
                          'DateRange',dateRange,'outputformat','timetable');   
api='http://api.thingspeak.com/channels/462617/feed.json?start=2018-12-22%2000:00:00&end=2019-01-10%2000:00:00&timezone=Australia%2FSydney';
s=webread(api);
T1=struct2table(s.feeds); %convert structure 2 table
tz=datetime(T1.created_at,'inputformat','yyyy-MM-dd''T''HH:mm:ssXXX','timezone','UTC');  %return UTC+11 in UTC time (no dst)                   
data1d.Timestamps=tz;   
[data2d] = thingSpeakRead(readChannelID2,'Fields',1:8,...
                          'DateRange',dateRange,'outputformat','timetable');   
api='http://api.thingspeak.com/channels/462618/feed.json?start=2018-12-22%2000:00:00&end=2019-01-10%2000:00:00&timezone=Australia%2FSydney';
s=webread(api);
T1=struct2table(s.feeds); %convert structure 2 table
tz=datetime(T1.created_at,'inputformat','yyyy-MM-dd''T''HH:mm:ssXXX','timezone','UTC');  %return UTC+11 in UTC time (no dst)                   
...
%% concatenate data
% time1=[time1a;time1b;time1c;time1d];
% time2=[time2a;time2b;time2c;time2d];
data1=[data1a;data1b;data1c;data1d];
data2=[data2a;data2b;data2c;data2d];

clear *a *b *c *d tz T1 s dateRange api startDate endDate
%%

save OAI_atmos41_2sensors data1 data2
